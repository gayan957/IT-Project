...
const CollectScheduleWaste = () => {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          throw new Error(
            "Waste prices not found - database may need initialization"
          );
        } else {
      ...
      if (result.success && result.data) {
        ...
        const priceData = result.data.find(
          (p) => p.wasteType === currentScheduleData.wasteType
        );
        if (priceData) {
          ...
          throw new Error(
            `Price not found for ${currentScheduleData.wasteType} waste`
          );
        }
      } else {
      ...
      const defaultPrice =
        defaultPrices[currentScheduleData.wasteType.toLowerCase()] || 25.0;
      ...
      toast.error(
        `Using default price for ${currentScheduleData.wasteType}: Rs. ${defaultPrice}/kg`
      );
    } finally {
      setLoading(false);
    }
  ...
  const orderDetails = {
    order_id: randomId,
    amount: totalPrice,
    first_name: currentScheduleData.userName.split(" ")[0],
    last_name: currentScheduleData.userName.split(" ")[1],
    email: currentScheduleData.userEmail || "test@test.com",
    phone: currentScheduleData.userPhone || "0712345678",
    address: currentScheduleData.address || "123, Main Street",
    city: "Colombo",
    country: "Sri Lanka",
    currency: "LKR",
    custom_1: currentScheduleData,
  };
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="mb-6">
              <h1 className="text-2xl font-bold text-gray-900 mb-2">
                Collect Schedule Waste
              </h1>
              <div className="text-sm text-gray-600">
                Schedule: {currentScheduleData.address} •{" "}
                {currentScheduleData.wasteType}
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-gray-900">
                    {new Date(
                      currentScheduleData.scheduledDate
                    ).toLocaleDateString()}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-gray-900">
                    {currentScheduleData.scheduledTime}
                  </p>
                </div>
                <div>
                  <span className="font-medium text-gray-700">User:</span>
                  <p className="text-gray-900">
                    {currentScheduleData.userName || "Unknown"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-gray-900">
                    {currentScheduleData.estimatedWeight || 0} kg
                  </p>
                </div>
                <div>
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              ...
              <div className="text-sm text-gray-600 space-y-1">
                <p>
                  ...
                  {currentScheduleData.address}
                </p>
                <p>
                  <span className="font-medium">Coordinates:</span>{" "}
                  {currentScheduleData.location?.lat},{" "}
                  {currentScheduleData.location?.lng}
                </p>
              </div>
            </div>
